...
int n,q,d; int adj[2*_+5]; bool vis[2*_+5];

int main() {
  IO("");
  in(n,q); 
  F0R(i,n) in(adj[i]),--adj[i]; 
  F0R(i,q) {
    int a,b; in(a,b),--a,--b; bool flag = 1;  
    queue<int> todo; todo.push(a); mem(vis,0); d = 0; 
    vis[a] = 1;
    while(!empt(todo)) {
      int p = todo.ft(); todo.pop(); 
      if(p == b) { flag = 0; out(d); break; }
      if(!vis[adj[p]]) vis[adj[p]] = 1,++d,todo.push(adj[p]);
    }
    if(flag) out(-1); 
  }
}
