...
int n,q,sal[2*_+5]; OST<pii> s; 

int main() {
  IO("");
	in(n,q); F0R(i,n) in(sal[i]), s.ins(mp(sal[i],i)); 
	while(q--) {
		char t; in(t); 
		if(t == '!') {
			int k,x; in(k,x),--k; s.ers(mp(sal[k],k)); 
			s.ins(mp(x,k)); sal[k] = x; 
		}
		else {
			int a,b; in(a,b); 
			int ans = s.ook(mp(b,INF)) - s.ook(mp(a,-INF));
			out(ans);  
		}
	}
}
