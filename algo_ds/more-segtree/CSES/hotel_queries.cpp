...
int nxtpw2(int k) {
  int res = 1; while(res < k) res *= 2; return res * 2; 
}
...
template<class T> struct SEG {
    ...
    template<typename it, typename = typename enable_if<is_iterator<it>::value>::type> 
    void init(it bg, it nd) { init(nxtpw2(distance(bg, nd))); move(bg, nd, begin(seg) + n); build(); }
...
int ans(int nd, int r) { //binary search on leftmost node >= r
  ...
  if(s.seg[s.lc(nd)] >= r) return ans(s.lc(nd),r); 
  else if(s.seg[s.rc(nd)] >= r) return ans(s.rc(nd),r); 
  else return -1; 
}

int main() {
  IO("");
  in(n,m); v.rsz(n); in(all(v)); s.init(all(v)); 
  F0R(i,m) {
    int r; in(r); out_(ans(1,r) + 1);
    if(ans(1,r) != -1) s.upd(ans(1,r),s.seg[ans(1,r)+s.n]-r); 
  } NL; 
}
