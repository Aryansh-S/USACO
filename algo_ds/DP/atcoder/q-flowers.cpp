...
const ll INFLL = 0x3f3f3f3f3f3f3f3f; // use ll!

int n; vector<array<int,3>> fl; // height, beauty, idx

struct SEG {
    const ll ID = 0; ll comb(ll a, ll b) { return max(a,b); }  
    int n; vector<ll> seg;
    ll get(int p) { return seg[p]; }
...
// dp[i] = max flower sum ending at flower idx i
// process flowers in increasing order of height
// for each flower f, dp[idx(f)] = max from dp[0...idx(f)-1] + beauty(f) 

int main() { 
	IO("");
	re(n); fl.resize(n); // height, beauty, idx
	F0R(i,n) re(fl[i][0]);
	F0R(i,n) re(fl[i][1]);
	F0R(i,n) fl[i][2] = i;
	sort(all(fl));
	dp.init(n); 
	trav(f,fl) dp.upd(f[2], dp.qry(0,f[2]-1) + f[1]);
	pr(dp.qry(0,n-1));
}
