...
int n,m,dp[_], ans = -INF; vi adj[_]; bool vis[_]; stack<int> ts;
...
signed main() { 
	IO("");	
	re(n,m); F0R(i,m) { int a,b; re(a,b),--a,--b; adj[a].eb(b); }
	F0R(i,n) if(!vis[i]) dfs(i); 
	
	while(sz(ts)) {
		trav(i,adj[ts.top()]) ckmax(dp[i],dp[ts.top()]+1);
		ts.pop(); 
	}

	trav(i,dp) ckmax(ans,i); 
	pr(ans);
}
