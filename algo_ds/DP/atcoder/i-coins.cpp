...
int n; ld p[_],ans,dp[_][_];

int main() { 
	IO("");	
	re(n); in(p,p+n); 
	// dp[heads][tails]
	dp[0][0] = 1; cout << setprecision(9); 
	F0R(i,n+1) F0R(j,n+1) if(i + j <= n) dp[i+1][j] += p[i+j] * dp[i][j], dp[i][j+1] += (1 - p[i+j]) * dp[i][j];
	F0R(i,n+1) if(i > n-i) ans += dp[i][n-i]; 
	pr(ans);
}
