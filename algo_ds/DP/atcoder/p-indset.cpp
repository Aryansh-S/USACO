...
ll n,dp[_][2]; vi adj[_];

ll dfs(int v, int p, bool c) { // 0 = black, 1 = white
	if(dp[v][c]) return dp[v][c];
	ll s = 1;
	trav(u,adj[v]) if(u != p) {
		s *= (dfs(u,v,!c) % MOD + c * dfs(u,v,c) % MOD);
		s %= MOD;
	}
	return dp[v][c] = s; 
}
...
int main() { 
	IO("");
	re(n); F0R(i,n-1) { int a,b; re(a,b),--a,--b; adj[a].eb(b), adj[b].eb(a); }
	// root at 0, dfs(vertex, parent, color)
	pr((dfs(0,0,0) + dfs(0,0,1)) % MOD); 
}
