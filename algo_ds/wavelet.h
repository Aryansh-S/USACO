...
// removes extra log factor, so good for HLD without updates
// credit: https://github.com/nilehmann/wavelet-tree/

template<class T> struct WSEG { //wavelet tree segtree
  ...
  WSEG(it bg, it nd, T maxel) : C((maxel + 1) * 2), s(maxel + 1) { 
