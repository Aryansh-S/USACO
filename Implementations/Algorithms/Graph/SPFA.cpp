...
//class T weight
    vector<pair<T,int> > adj[SZ]; T dist[SZ]; bool inq[SZ]; int mom[SZ]; 
    void add(int a, int b, T w){adj[a].pb(mp(w,b)); adj[b].pb(mp(w,a));}
	void add_(int a, int b, T w){adj[a].pb(mp(w,b));} //if edge is directed
    void upd(int start){
        F0R(i,SZ) dist[i]=(i!=start)?INF:0,inq[i]=0;
        queue<pair<T, int> > q; q.push(mp(0,start));
        inq[start]=1;
        while(!q.empty()){
            auto curr=q.front(); q.pop(); inq[curr.s]=0;
            trav(v,adj[curr.s]){
                what(sz(q)); exit(0);
                if(dist[v.s]>dist[curr.s]+v.f){
                    dist[v.s]=dist[curr.s]+v.f;
                    if(!inq[v.s]){inq[v.s]=1; q.push(mp(v.f,v.s));}
                }
            }
        }
        out("TEST"); exit(0);
