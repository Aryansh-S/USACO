...
int main(){
    ...
    in(n); 
    F0R(i,n) in(pts[i].f,pts[i].s); sort(pts,pts+n); 
    minl[0]=pts[0].s, maxr[n-1]=pts[n-1].s, ans=1;

    //min/max dp
    F1R(i,n-1) minl[i]=min(minl[i-1],pts[i].s); 
    R0F(i,n-1) maxr[i]=max(maxr[i+1],pts[i].s);

    F0R(i,n-1) ans+=(minl[i]>maxr[i+1]);
    out(ans);
