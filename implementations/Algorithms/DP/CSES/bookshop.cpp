...
int n,x,pr[1005],pg[1005],dp[1005][_+5]; //first i books, max j dollars, start states 0

int main() {
	IO("");
  in(n,x); F1R(i,n) in(pr[i]); F1R(i,n) in(pg[i]); 
  F0R(i,1000) F1R(j,_) dp[i+1][j] = pr[i+1] > j ? dp[i][j] : max(dp[i][j],dp[i][j-pr[i+1]]+pg[i+1]);
    //can we include the i+1th weight?
  out(dp[n][x]);
}
