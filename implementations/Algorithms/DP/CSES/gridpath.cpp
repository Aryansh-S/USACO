...
int n; mi dp[2000][2000]; bool gr[2000][2000];

int main() {
	IO("");
  in(n); F0R(i,n) F0R(j,n) { char c; in(c); gr[i][j] = (c=='.'); }
  if(!gr[0][0]) { out(0); return 0; }
  dp[0][0] = 1; F0R(i,n) F0R(j,n) dp[i+1][j] += (gr[i+1][j]) ? dp[i][j] : 0, dp[i][j+1] += (gr[i][j+1]) ? dp[i][j] : 0;
  out(dp[n-1][n-1]);
}
