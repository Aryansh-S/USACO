#include <bits/stdc++.h>
using namespace std;
//a single MST with the prim struct setting used edges to 0 suffices
...
const int mx=1e3+5;
int n,m; db adj[mx][mx]; pdb pts[mx]; Prim<db,mx> p; 
...
int main(){
	...
	in(n,m); 
	F0R(i,n) {int x,y; in(x,y); pts[i]=mp(x,y);}
	F0R(i,n) F0R(j,n) adj[i][j]=dist(pts[i],pts[j]);
	F0R(i,m) {int x,y; in(x,y),x--,y--; adj[x][y]=adj[y][x]=0;}
	F0R(i,n) F0R(j,n) p.add_(i,j,adj[i][j]);
	p.upd();
	out(p.sum());
