#include <bits/stdc++.h>
using namespace std;
//just use the Prim MST struct and add a function to check for existence
...
template<class T, int SZ> struct Prim{ // T is type weight
    ...
    bool exi(){bool ret=1; F0R(i,n) if(key[i]==INF) ret=0; return ret;}
};
...
int main(){
	...
	in(n,c); F0R(i,n){int x,y; in(x,y),pts[i]=mp(x,y);}
	F0R(i,n) F0R(j,n) if(dist(pts[i],pts[j])>=c) p.add_(i,j,dist(pts[i],pts[j]));
	p.upd(); 
	out((p.exi())?p.sum():-1);
