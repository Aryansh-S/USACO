...
#define PNAME "cardgame"

int n,ans; set<int> hia, lowa, totb;

int main(){
	cin.tie(0)->sync_with_stdio(0); if(fopen(PNAME ".in","r")){freopen(PNAME ".in","r",stdin); freopen(PNAME ".out","w",stdout);}
	in(n);
	F1R(i,2*n) totb.ins(i);
	F0R(i,n/2) {int x; in(x); hia.ins(x); totb.ers(x);}
	F0R(i,n/2) {int x; in(x); lowa.ins(x); totb.ers(x);}
	trav(a,lowa){
