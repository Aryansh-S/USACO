...
template<class T> struct SEG{ //UPD literally updates!!
	...
	T comb(T a, T b) {return (a==orz)?((b==orz)?0:b):((b==orz)?a:(choice=='m')?min(a,b):(choice=='s')?a+b:(a^b));}
	int n; vector<T> seg;
...
int main(){
	...
	in(n,q); seg.choose('M'); seg.init(n+5);
	F0R(i,n){ll x; in(x); seg.upd(i,x);}
	F0R(i,q){ll x,y; in(x,y); x--,y--; out(seg.query(x,y));}
	return 0;
}
