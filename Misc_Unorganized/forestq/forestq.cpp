...
#define PNAME "forestq"

ll n,q; BIT<ll,1010,1010> bit;

int main(){
	cin.tie(0)->sync_with_stdio(0); if(fopen(PNAME ".in","r")){freopen(PNAME ".in","r",stdin); freopen(PNAME ".out","w",stdout);}
	in(n,q); F0R(i,n) F0R(j,n) {char c; in(c); if(c=='*') bit.upd(i+1,j+1,1);}
	F0R(i,q) {ll x1,y1,x2,y2; in(x1,y1,x2,y2); out(bit.query(x1,x2,y1,y2));}
	return 0;
}
