...
int n,h[_],ls[_],rs[_],ans; oset<int> cc; 

struct BIT {
	int bit[_ + 1] {}; 
	void upd(int i, int x) { for(; i <= _; i += i & -i) bit[i] += x; }
	int qry(int i) { 
		int sum = 0; 
...
} L, R; // 1 indexed, inclusive, l,r

int main() { 
	IO("bphoto");	
	re(n); in(h,h+n); 
	F0R(i,n) cc.ins(h[i]); 
	F0R(i,n) h[i] = cc.ook(h[i]); 
	// out(h,h+n); 
	F0R(i,n) L.upd(h[i]+1,1), ls[i] = L.qry(h[i]+2,n);
	R0F(i,n) R.upd(h[i]+1,1), rs[i] = R.qry(h[i]+2,n); 
	F0R(i,n) if(max(ls[i],rs[i]) > 2 * min(ls[i],rs[i])) ++ans;
	pr(ans);
}
