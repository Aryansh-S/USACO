...
int dist[_][26], dp[_][26], psum[_][26], mins[_];

int main() {
  IO("cowmbat");  
  in(n,m,k,s); 
  F0R(i,m) F0R(j,m) { int x; in(x); f.add_(i,j,x); } //directed edge
  f.upd();
  F1R(i,n) F0R(j,m) dist[i][j] = f.query(s[i-1]-'a',j), psum[i][j] = psum[i-1][j]+dist[i][j];
  memset(dp,0x3f,sizeof(dp)), memset(mins,0x3f,sizeof(mins));
  mins[0]=0; //base case
  F1R(i,n) F0R(j,m) {
    int cmp = dp[i-1][j]+dist[i][j];
    if(i>=k) ckmin(cmp,psum[i][j]-psum[i-k][j]+mins[i-k]); 
    ckmin(dp[i][j],cmp);
    ckmin(mins[i],dp[i][j]);
  }
  out(mins[n]); 
