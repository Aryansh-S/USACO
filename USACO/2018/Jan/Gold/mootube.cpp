...
namespace aryansh { 
	namespace type_macros {
		...
		template<class T, class cmp = less_equal<T>> using MOST //allow multi
			= tree<T, null_type, cmp, rb_tree_tag,tree_order_statistics_node_update>; 
		template<class T, class U, class cmp = less<T>> using OSM 
			= tree<T, U, cmp, rb_tree_tag,tree_order_statistics_node_update>;
		template<class T, class U, class cmp = less_equal<T>> using MOSM //allow multi
			= tree<T, U, cmp, rb_tree_tag,tree_order_statistics_node_update>;
...
int n,q; DSU d; 
maxpq<ar<int,3>> e; //r, a, b 
vector<ar<int,3>> qr; //k, v, i
int ans[_];
...
int main() {
  ...
  F0R(i,n-1) { int p,q,r; in(p,q,r); e.push(ar<int,3>{r,p,q}); }
  F0R(i,q) { int k,v; in(k,v); qr.pb(ar<int,3>{k,v,i}); }
  
  sort(all(qr)); reverse(all(qr));
  
  trav(t,qr) { 
    while(!e.empty() && e.top()[0] >= t[0]) { d.unite(e.top()[1], e.top()[2]); e.pop(); } 
    ans[t[2]] = d.size(t[1]) - 1;
  } 
