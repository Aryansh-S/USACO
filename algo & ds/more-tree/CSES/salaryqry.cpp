...
template<class T0> struct SEG { //merge sort tree
    ...
    T0 qry(int l, int r, T0 a, T0 b) { //query for the existence of x in [l,r]
      T0 ret = 0; 
      for(l += n, r += n + 1; l < r; l /= 2, r /= 2) {
          if(l & 1) {
            auto L = get(l++); 
            //ret = comb(ret,binary_search(all(L),x)); 
            ret += ub(all(L),b) - lb(all(L),a); 
          }
          if(r & 1) {
            auto R = get(--r); 
            //ret = comb(ret,binary_search(all(R),x));
            ret += ub(all(R),b) - lb(all(R),a); 
          }
      }
      return ret;
    }
...
int main() {
  ...
  while(q--) {
    char t; in(t); 
    if(t == '!') {
      int k,x; in(k,x),--k; s.upd(k,x);
    }
    else {
      int a,b; in(a,b); out(s.qry(0,n-1,a,b)); 
    }
  }
}
