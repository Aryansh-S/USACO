...
template<int SZ, bool EDGE = 1> struct HLD { //add all edges, then init
  ...
  void init(int R = 0) {
    ...
    rngset<T> tree; 
    const bool ID = 0; 
    bool comb(bool a, bool b) { return a || b; }
  #endif
  ...
  #endif
  bool qrypath(int x, int y, int v) { 
      bool res = ID; processPath(x,y,[this,&res,&v](int l, int r) { 
      res = comb(res,tree.qry(l,r,v)); });
      return res; 
  }
  #ifdef RANGE_UPD
...
int n,m; vi cow; HLD<_,0> h; 

int main() {
  IO("milkvisits");
  in(n,m); cow.rsz(n); F0R(i,n) in(cow[i]); 
  F0R(i,n-1) { int x,y; in(x,y),--x,--y; h.add(x,y); }
  h.init(); 
  F0R(i,n) h.upd(i,cow[i]);
  while(m--) {
    int a,b,c; in(a,b,c),--a,--b; 
    cout << h.qrypath(a,b,c); 
  } NL; 
}
